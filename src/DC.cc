...
//:::::::::::::::::::::::::::optical properties for silicon::::::::::::::::::::::::::::::::::::::::::::::
 // =========================================================================
    ...
    // Hamamatsu VUV-3 MPPC photon detection efficiency (paper Fig. 3, dashed blue curve)
    // Energies correspond to SiPM[] array: 1.13, 1.24, 1.38, 1.55, 1.77, 2.07, 2.48, 3.10, 4.13, 6.20, 6.53 eV
    //                              i.e. λ: 1100, 1000,  900,  800,  700,  600,  500,  400,  300,  200,  190  nm
    G4double efficiency[iNbEntries_Si] = {
     0.01,  // 1100 nm - negligible in IR
     0.02,  // 1000 nm
     0.02,  //  900 nm
     0.03,  //  800 nm
     0.05,  //  700 nm
     0.06,  //  600 nm - visible band of CF4 (peak ~620 nm)
     0.08,  //  500 nm
     0.18,  //  400 nm - UV band of CF4 (peak ~290 nm)
     0.22,  //  300 nm - near UV peak
     0.28,  //  200 nm - VUV, approaching VUV-3 peak
     0.30   //  190 nm - VUV-3 peak sensitivity (~30%)
    };
...
void DC::Sphereball(G4double position) {
    G4double sphereRadius = 5.0*cm;  
    
    G4Sphere* sphereball = new G4Sphere("sphereball",
                                         0,            // inner radius
                                         sphereRadius, // outer radius = 10mm
                                         0*deg, 360*deg,
                                         0*deg, 180*deg);
    
    G4LogicalVolume* sphereVolume = new G4LogicalVolume(sphereball,
                                                         b_polyethylene,  
                                                         "Sphere");
...
void DC::ConstructResolutionPhantom()
{
    ...
    G4double plateThickness = 10*cm;  // thicker for fast neutrons
    G4double plateSize      = 100*mm; // match your detector size
    ...
    // Holes of different sizes to test resolution
    ...
    // Place between source and detector
    // Source at z=-19mm, ball at z=200mm, PPAC at z=700mm
    // Put phantom at z=100mm (midway, good position)
...
void DC::ConstructValve(G4double position) {
    ...
    G4double valveLength  = 100.0 * mm;
    G4double valveRadius  =  25.0 * mm;
    G4double cavityRadius =  14.9 * mm;
    G4double stemRadius   =   6.0 * mm;
    ...
    // BUG FIX: hole radius must match cavityRadius (was hardcoded 10.0*mm)
    // The +0.1*mm overrun prevents a zero-thickness coincident surface
    G4Tubs* solidValveHole = new G4Tubs("ValveHole",
                                         0, cavityRadius,
                                         valveLength / 2 + 0.1*mm,
                                         0, 360*deg);

    G4SubtractionSolid* solidValveBody = new G4SubtractionSolid(
        "ValveBody", solidValveOuter, solidValveHole);

    G4LogicalVolume* logicValveBody =
        new G4LogicalVolume(solidValveBody, steel, "ValveBody");

    new G4PVPlacement(nullptr,
                      G4ThreeVector(0, 0, position),
                      logicValveBody, "ValveBody",
    ...
    G4LogicalVolume* logicCavity =
        new G4LogicalVolume(solidCavity, water, "Cavity");
    ...
    // ── Aluminium stem (daughter of cavity) ──────────────────────────────────
    // Stem runs along Y across the bore.
    // BUG FIX: half-length derived from cavityRadius so it exactly fits
    // the bore without protruding into steel (the -0.1*mm is a geometry guard)
    G4double stemHalfLen = cavityRadius - 0.1*mm;
    ...
    G4LogicalVolume* logicStem =
        new G4LogicalVolume(solidStem, Aluminium, "ValveStem");

    G4RotationMatrix* stemRot = new G4RotationMatrix();
    stemRot->rotateX(90*deg);   // lay along Y so it crosses the beam bore

    new G4PVPlacement(stemRot,
                      G4ThreeVector(0, 0, 0),
                      logicStem, "ValveStem",
                      logicCavity,             // daughter of cavity, not world
                      false, 0, true);

    // ── Visualisation ────────────────────────────────────────────────────────
    G4VisAttributes* steelVis = new G4VisAttributes(G4Colour(0.6, 0.6, 0.6));
    steelVis->SetVisibility(true);
    steelVis->SetForceSolid(true);
    logicValveBody->SetVisAttributes(steelVis);

    G4VisAttributes* waterVis = new G4VisAttributes(G4Colour(0.0, 0.0, 1.0));
    waterVis->SetVisibility(true);
    waterVis->SetForceSolid(true);
    logicCavity->SetVisAttributes(waterVis);

    G4VisAttributes* stemVis = new G4VisAttributes(G4Colour(1.0, 0.0, 0.0));
    stemVis->SetVisibility(true);
    stemVis->SetForceSolid(true);
...
void DC::ConstructLaboratory()
{
  ...
  G4double converter_z =  0.05 mm; // 0.75*mm;
  ...
  auto pShield = new G4PVPlacement(0,
                                    G4ThreeVector(0.0, 0.0,-1.5*mm- 0.75*mm -1.6*um+70.0*cm),
                                    lShield,
                                    "Shield",
                                    experimentalHall_log,
                                    false,
  ...
  //===========================================================================
  // Collimator Frame - Vertical bars (left/right)
  //===========================================================================
  ...
  // Left side vertical collimators
  for (int a=0;a<34;a++) 
  {
    collf_phys = new G4PVPlacement(0,G4ThreeVector(-50.0*mm-collf_x,(-49.5+a*3)*mm,0.0),
                                    collf_log,"collf_Vol",PPAC_log,false,a,true);  
  }
  
  // Right side vertical collimators
  for (int a=0;a<34;a++) 
  {
    collf_phys = new G4PVPlacement(0,G4ThreeVector(50.0*mm+collf_x,(-49.5+a*3)*mm,0.0),
                                    collf_log,"collf_Vol",PPAC_log,false,a+33,true);  
  }

  //===========================================================================
  // Collimator Frame - Horizontal bars (top/bottom)
  //===========================================================================
  ...
  // Bottom side horizontal collimators
  for (int b=0;b<34;b++) 
  {
    coll_phys = new G4PVPlacement(0,G4ThreeVector((-49.5+b*3)*mm,-50.0*mm-coll_y,0.0),
                                   coll_log,"coll_Vol",PPAC_log,false,b,true);  
  }
  
  // Top side horizontal collimators
  for (int b=0;b<34;b++) 
  {
    coll_phys = new G4PVPlacement(0,G4ThreeVector((-49.5+b*3)*mm,50.0*mm+coll_y,0.0),
                                   coll_log,"coll_Vol",PPAC_log,false,b+33,true);  
  }
  ...
  for (int c=0;c<33;c++) 
  {
    sensor_phys = new G4PVPlacement(0,
                                     G4ThreeVector((-50.0*mm - collimatore)+sensor_x,
                                                   (-49.5+c*3+1.5)*mm,
                                                   0.0),
                                     sensor_log1,
                                     "sensor_Vol1",
  ...
  for (int d = 0; d < 33; d++) 
  {
    ...
    sensor_phys2 = new G4PVPlacement(0,
                                      G4ThreeVector(sensor_x_position,
                                                    (-50.0 * mm - collimatore) + siPM_y,
                                                    0.0),
                                      sensor_log3,
                                      "sensor_Vol3",
                                      PPAC_log,
  ...
  cathode_phys = new G4PVPlacement(0,
                                    G4ThreeVector(0.0,0.0,1.5*mm+0.8*um+70.0*cm),
                                    cathode_log,
                                    "cathode_Vol",
                                    experimentalHall_log,
                                    false,
  ...
  cathodeAl_phys = new G4PVPlacement(0,
                                      G4ThreeVector(0.0,0.0,-0.8*um+0.05*um),
                                      cathodeAl_log,
                                      "cathodeAl_Vol",
                                      cathode_log,
                                      false,
  ...
  // surface absorbing (Teflon collimators)
